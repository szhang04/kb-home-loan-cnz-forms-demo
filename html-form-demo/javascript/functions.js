...
function Repair_XML() {
    ...
    borrower_xpath = '/Package/Content/Application/PersonApplicant[@ApplicantType="Borrower"]';
    ...
    var nonrealestateassets = xpath.select(nonrealestateassets_xpath, lixi_package.node);
    ...
    purchases_xpath = '/Package/Content/Application/RealEstateAsset[@Transaction="Purchasing"]';
...
function Escape_XML_Hex_To_HTML(my_string) {
    return my_string.replaceAll("%26", "&amp;")
        .replaceAll("%3C", "&lt;")
        .replaceAll("%3E", "&gt;")
        .replaceAll("%22", "&quot;")
        .replaceAll("%27", "&apos;")
...
function Set_Default_Values() {
    ...
    var inputs = document.querySelectorAll('#myTabContent input, #myTabContent select');
    for (var kindex = 0; kindex < inputs.length; kindex++) {
        ...
        if (placeholder_value) {
            ...
            var full_xpath = this_input.getAttribute("xpath") + '="' + placeholder_value + '"';
            xml_string = insert_xpath_recursive(full_xpath, xml_string)[1];
        }
        if (this_input.getAttribute("xpath") == "/Package/SchemaVersion/@LIXITransactionType") {
            var xpath = '//xs:attribute[@name="LIXITransactionType"]/@fixed';
            var LIXITransactionType = schema.evaluate(xpath, schema, resolver, XPathResult.ANY_TYPE, null).iterateNext().textContent;
            this_input.setAttribute("value", LIXITransactionType);
            var full_xpath = '/Package/SchemaVersion/@LIXITransactionType="' + LIXITransactionType + '"';
            xml_string = insert_xpath_recursive(full_xpath, xml_string)[1];
        };
        if (this_input.getAttribute("xpath") == "/Package/SchemaVersion/@LIXIVersion") {
            var xpath = '//xs:attribute[@name="LIXIVersion"]/@fixed';
            var LIXIVersion = schema.evaluate(xpath, schema, resolver, XPathResult.ANY_TYPE, null).iterateNext().textContent;
            this_input.setAttribute("value", LIXIVersion);
            var full_xpath = '/Package/SchemaVersion/@LIXIVersion="' + LIXIVersion + '"';
            xml_string = insert_xpath_recursive(full_xpath, xml_string)[1];
        };
    }
}
...
function get_all_paths() {
    ...
    var paths_result = schema.evaluate(xpath, schema, resolver, XPathResult.ANY_TYPE, null);
...
function Enable_Dependant_Fields(lixi_package, container_id) {
    ...
    for (var jindex = 0; jindex < dependentFields.length; jindex++) {
        ...
        var prerequisite = "boolean(" + dependentField.attributes['prerequisite'].value + ")";
        var result = xpath.select(prerequisite, lixi_package.node)
        if (result) {
...
function Display_Message(lixi_package) {
    ...
    var results = xpath.select("/Package/Attachment/InlineAttachment", lixi_package_temp.node);
    ...
    try {
        ...
        old_context_json = hightlight_compare(context_json, old_context_json, false);
    } catch (e) {
    ...
    if (show_JSON == false) {
        ...
        document.getElementById("highlight-content").classList.remove('language-json');
        ...
        document.getElementById("highlight-content").classList.remove('language-xml');
...
function update_syntax_hightlight(text, start, end) {
    ...
    if (end > 0)
        document.getElementById("highlight").setAttribute("data-line", start + "-" + end);
...
function hightlight_compare(new_context, old_context, isXML) {
    ...
    if (old_context === new_context) {
        ...
        while (start < Num_lines) {
            if (arr_new[start] !== arr_old[start])
                break;
            start++;
        }
        start++;
        ...
        for (let i = 1; i < Num_lines; i++) {
            if (arr_new[arr_new.length - i] !== arr_old[arr_old.length - i])
                break;
            end--;
        }
...
var json = {
    ...
    "package": {
        "content": {
            "application": {
                "overview": {
                    "@brokerApplicationReferenceNumber": "{{$guid}}",
                    "@ExpectedSettlementDate": "2023-08-24"
                },
                "Address": [
                    {
                        "@City": "Auckland",
                        "@Country": "NZ",
                        "@NewZealandPostCode": "2019",
                        "@Suburb": "Flat Bush",
                        "@Type": "Standard",
                        "@UniqueID": "AD001",
                        "Standard": {
                ...
                "Household": [
                    {
                        ...
                        "@UniqueID": "Family_7006830",
                        "Dependant": [
                            {
                                "@Age": 2,
                                "@DateOfBirth": "2020-03-03",
                                "@Name": "Keilah Terrobias",
                                "@UniqueID": "Person_8314477"
                            },
                            {
                                "@Age": 9,
                                "@DateOfBirth": "2013-04-04",
                                "@Name": "Nico Terrobias",
                                "@UniqueID": "Person_8314478"
                            },
                            {
                                "@Age": 12,
                                "@DateOfBirth": "2010-05-05",
                                "@Name": "John Medenilla",
                                "@UniqueID": "Person_8314479"
                            }
                        ]
                    }
                ],
                "PersonApplicant": [
                    {
                        ...
                        "@Citizenship": "NZ",
                        "@CountryOfBirth": "PH",
                        "@ResidencyStatus": "Permanently In New Zealand",
                        "Contact": {
                            "CurrentAddress": {
                                "@HousingStatus": "Own Home Mortgage",
                                "@StartDate": "1996-04-24",
                                "@X_ResidentialAddress": "AD001",
                                "Duration": {
                                    "@Length": 1,
                                    "@Units": "Days"
                                }
                            ...
                            "EmailAddress": [
                                {
                                    "@Email": "jomar.terrobias@kiwibank.co.nz"
                                }
                            ],
                            "HomePhone": {
                                "@Number": "98765432"
                        ...
                        "Employment": [
                            {
                                "SelfEmployed": {
                                    "@ANZSCOOccupationCode": "452499",
                                    "@Basis": "Full Time",
                                    "@Status": "Primary",
                                    "@Occupation": "Tennis Player",
                                    "@x_Employer": "RC001",
                                    "BusinessIncomeYearToDate": {
                                        "@ProfitAfterTax": 2844434.0,
                                        "@ProfitBeforeTax": 5122783.0,
                ...
                "SalesChannel": {
                    "LoanWriter": {
                        "@FirstName": "Daniel",
                        "@Surname": "Head",
                        "@UniqueID": "Family_7006829"
                    }
                },
                "NonRealEstateAsset": [
                    {
                        "@ToBeSold": "No",
                        "@ToBeUsedAsSecurity": "No",
                        "@Type": "Financial Asset",
                        "@UniqueID": "FinancialInfo_12478912",
                        "EstimatedValue": {
                ...
                "RealEstateAsset": [
                    {
                        ...
                        "@Transaction": "Owns",
                        "@UniqueID": "SecurityInfo_4488902",
                        "@x_Address": "AD001",
                        "ContractDetails": {
                            "@LicencedRealEstateAgentContract": "No"
                        },
                        "EstimatedValue": {
                            "@EstimateBasis": "Applicant Estimate",
                            "@Value": 420000
                        ...
                        "PercentOwned": {
                            ...
                            "Owner": [
                                {
                                    "@Percent": 50,
                                    "@x_Party": "Family_8314475"
                                },
                                {
                                    "@Percent": 50,
                                    "@x_Party": "Family_8314476"
                                }
                            ]
                        },
                        "PropertySearchValidation": {
                            "@OwnerNames": "JomarTerrobias"
                        },
                        "PropertyType": {
                            "@PropertyTypeName": "Std Residential"
                        },
                        "RentalIncome": [
                            {
                                "@EvidenceOfTenancy": "Yes",
                                "@Frequency": "Monthly",
                                "@RentalAmount": 0
                            }
                        ],
                ...
                "LoanDetails": [
                    {
                        "@AmountRequested": 6500000.0,
                        "@ProposedAnnualInterestRate": 6.99,
                        "@LoanType": "Line of Credit Home Loan",
                        "Borrowers": {
                            "Owner": [
                                {
                                    "@x_Party": "PERSON1"
                        ...
                        "LendingPurpose": [
                            {
                                "@ABSLendingPurposeCode": "ABS-121",
                                "@PurposeOfFunds": "Purchase of Dwelling"
                            }
                        ],
                        "LoanPurpose": {
                            "@PrimaryPurpose": "Owner Occupied"
                        ...
                        "Term": {
                            ...
                            "@TotalTermDuration": 55,
                            "@TotalTermType": "Amortisation Term",
                            "@TotalTermUnits": "Weeks",
                            "@PaymentType": "Interest Only",
                            "@PaymentTypeDuration": 0,
                            "@PaymentTypeUnits": "Weeks"
                        }
                    }
                ],
                "@uniqueID": "{{$guid}}"
        ...
        "Recipient": [
            {
                ...
                "@LIXICode": "UQWDVN6",
                "@RoutingCode": "Sample Routing Code",
                "@SequenceNumber": "12322",
                "Software": {
...
function SubmitToAPI() {
    ...
    json.package.content.application["@uniqueID"] = UUIDGeneratorBrowser();
    json.package.content.application.overview["@brokerApplicationReferenceNumber"] = UUIDGeneratorBrowser();
...
// function sendPostRequest_fetch(data) {
//     //var url = "http://localhost:5013/homeloan/v1";
//     var url = "https://lending-service-accept.apps.5e06.cip-non-production.nonprod.internal.aws.kiwibank.co.nz/homeloan/v1",
...
function ExportPackage() {
    if (show_JSON) {
        ...
        hiddenElement.href = 'data:attachment/text,' + encodeURI(Escape_JSON_Hex_To_JSON(converted_json));
        ...
        var hiddenElement = document.createElement('a');
        hiddenElement.href = 'data:attachment/text,' + encodeURI(Escape_XML_Hex_To_HTML(formattedXML));
        hiddenElement.target = '_blank';
...
function Update_Cross_Reference_Fields(lixi_package) {
    ...
    var cross_ref_fields = document.querySelectorAll(active_tab_id + ' .entity-selector');
    ...
    for (var j = 0; j < cross_ref_fields.length; j++) {
        ...
        var targets_str = this_input.attributes['targets'].value.replace(/\'/g, "\"");
        ...
        for (var k = 0; k < references.length; k++) {
            ...
            else if (referenceXPath == "/Package/Content/Application/PersonApplicant") {
                entity_type = "Person Applicant";
            }
            else if (referenceXPath == "/Package/Content/Application/CompanyApplicant") {
                entity_type = "Company Applicant";
            }
            else if (referenceXPath == "/Package/Content/Application/RealEstateAsset") {
                entity_type = "Real Estate Asset";
            }
            else if (referenceXPath == "/Package/Content/Application/NonRealEstateAsset") {
                entity_type = "Non Real Estate Asset";
            }
            else if (referenceXPath == "/Package/Content/Application/RelatedCompany") {
            ...
            while (node) {
                ...
                for (var l = 0; l < string_components.length; l++) {
                    ...
                    var string_component_xpath_without_variable = string_component_xpath.replace("$UniqueID", uniqueID);
                    if (xpath.select(string_component_xpath_without_variable, node)[0]) {
                        entity_string += xpath.select(string_component_xpath_without_variable, node)[0].value + " ";
                    }
                }
...
function create_drop_down_menu(this_input, entities, selected_option) {
    ...
    var newUUID = "x" + uuid();
    if (modal_id != "None") {
        select_element.setAttribute("onchange", "if (this.selectedIndex == 1) { New_Entity_Modal('" + modal_id + "','" + newUUID + "'); } else {$('.tooltip').tooltip('hide');}");
    }
...
function New_Entity_Modal(this_modal_id, newUUID) {
    ...
    for (var index = 0; index < rows.length; index++) {
        ...
        if (this_row.hasAttribute("prerequisite-variable")) {
            var prerequisite_with_variable = this_row.getAttribute("prerequisite-variable");
            this_row.setAttribute('prerequisite', prerequisite_with_variable.replace("$UniqueID", newUUID));
    ...
    for (var index = 0; index < inputs.length; index++) {
        ...
        var xpath_with_new_uniqueID = xpath_with_variable.replace("$UniqueID", newUUID);
        ...
        if (this_input.hasAttribute("prerequisite-variable")) {
            var prerequisite_with_variable = this_input.getAttribute("prerequisite-variable");
            this_input.setAttribute('prerequisite', prerequisite_with_variable.replace("$UniqueID", newUUID));
        }
...
function Import_Field_Values_From_XML(lixi_package) {
    ...
    var inputs = document.querySelectorAll(
        active_tab_id + " input, " +
        active_tab_id + " textarea, " +
        active_tab_id + " select, " +
        active_tab_id + " img, " +
        active_tab_id + " iframe, " +
        active_tab_id + " embed, " +
        active_tab_id + " object");
    ...
    for (var index = 0; index < inputs.length; index++) {
        ...
        if (input.hasAttribute("xpath")) {
            ...
            if (this_xpath.endsWith("text()")) {
                mediatype_xpath = this_xpath.replace("InlineAttachment/text()", "@MediaType")
                ...
                if ((mediatype_result.length > 0) && (result.length > 0)) {
                    ...
                    if (
                        ...
                        iframe.src = "data:" + mediatype + ";base64," + result[0].textContent;
                    }
                    else if (
                        ...
                        image.src = "data:" + mediatype + ";base64," + result[0].textContent;
                    }
                }
...
function Validate() {
    ...
    if (result.errors == null) {
        ...
        result.errors.forEach(function (error) {
            validation_results += String(error.replace("file_0.xml:", "").replace("element", "").replace("Schemas validity error :", "").split(/\:(.+)/)[1]) + "\n\n"
        });
        document.getElementById("validation-results").innerText = validation_results;
    }
}
...
function Toggle_Sticker(trigger) {
    ...
    if (sticker.classList.contains('display-hide'))
        ...
        sticker.classList.add("display-hide");
}
...
function Toggle_resize_handle_y() {
    ...
    if (isDisplayCode && isDisplayShard) {
        resize_handle.classList.remove("resize-hide");
        document.getElementById('show-code-container').setAttribute("style", "max-height:90%");
        ...
        resize_handle.classList.add("resize-hide");
        document.getElementById('show-code-container').setAttribute("style", "height:100%");
        document.getElementById('show-shard-container').setAttribute("style", "height:100%");
    }
}

function HideNavbar() {
    subMenu = document.querySelectorAll('.navbar .top-left .dropdown-wrap .dropdown-content');
